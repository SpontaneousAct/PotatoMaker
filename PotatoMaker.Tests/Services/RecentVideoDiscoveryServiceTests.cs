...
public sealed class RecentVideoDiscoveryServiceTests
{
    ...
    public async Task GetRecentVideos_FiltersSortsAndLimitsSupportedFiles()
    {
        ...
        try
        {
            ...
            [
                CreateFile(directory, "clip-01.mp4", DateTime.UtcNow.AddMinutes(-10)),
                CreateFile(directory, "clip-02.mkv", DateTime.UtcNow.AddMinutes(-9)),
                CreateFile(nestedDirectory, "clip-03.mov", DateTime.UtcNow.AddMinutes(-8)),
                CreateFile(directory, "clip-04.webm", DateTime.UtcNow.AddMinutes(-7)),
                CreateFile(nestedDirectory, "clip-05.avi", DateTime.UtcNow.AddMinutes(-6)),
                CreateFile(directory, "clip-06.mp4", DateTime.UtcNow.AddMinutes(-5)),
                CreateFile(nestedDirectory, "clip-07.mkv", DateTime.UtcNow.AddMinutes(-4)),
                CreateFile(directory, "clip-08.mov", DateTime.UtcNow.AddMinutes(-3)),
                CreateFile(nestedDirectory, "clip-09.webm", DateTime.UtcNow.AddMinutes(-2))
            ];
            ...
            Assert.Equal("clip-08.mov", results[1].FileName);
